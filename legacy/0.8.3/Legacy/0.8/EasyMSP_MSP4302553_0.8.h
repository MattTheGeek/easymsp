...
static void hpwm(register unsigned int port, register unsigned int freq, register unsigned int duty)
{
	//Port is a timer pin
	//Freq is how fast the PWM ocsolates
    //Duty 
    
    //Calculate Vaules for timer
    
    register unsigned int timercnt =  //Some complex math
	register unsigned int pwmdutystart =
	register unsigned int pwmdutyend = 
	TACTL = TACLR;
	TACTL = TASSEL_0 + ID_0 + MC_2;
	
	TAR = timercnt;
	TACCR1 = pwmdutystart;//PWM PulseEnd
	TACCR0 = pwmdutyend; //PWM PulseStart
	
	TACCTL0 = OUTMOD_3;
	TACCTL1 = OUTMOD_3;

static void uartOut(register unsigned char *data)
{
	
}

static unsigned int uartIn()
{
	register unsigned int data=0;
	
 	data = uartBuffer[uartIndex];
...
static void uartConfig(register unsigned int baud, register unsigned int inverted, register unsigned int stopbits, register unsigned int parity)
{
